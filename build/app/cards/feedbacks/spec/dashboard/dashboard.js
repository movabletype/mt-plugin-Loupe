describe("feedbacks", function () {
  ...
  beforeEach(function () {
    ...
    runs(function () {
      requireModuleAndWait(['cards/feedbacks/dashboard/dashboard']);
    });

    runs(function () {
      var dashboardOrig = require('cards/feedbacks/dashboard/dashboard');
      undefRequireModule('cards/feedbacks/dashboard/dashboard');
      define('cards/feedbacks/dashboard/dashboard', [], function () {
      ...
      requireModuleAndWait(['cards/feedbacks/models/comments_collection', 'cards/feedbacks/dashboard/dashboard']);
    ...
    runs(function () {
      initController(Controller, controller, function (data) {
        ...
        Dashboard = require('cards/feedbacks/dashboard/dashboard');
      });
    });
  ...
  describe("dashboard/main", function () {
    ...
    it("initialize with cached collection synced already", function () {
      ...
      runs(function () {
        ...
        var cache = require('js/cache');
        cache.set(1, 'feedbacks_comments', collection);
        dashboard = new Dashboard(initData);
      });
    ...
    it("shows default message when collection has no item", function () {
      window.Mock.throwListCommentItemsLength = 0;
      window.Mock.throwListCommentItems = [];
      ...
      runs(function () {
        ...
        expect(item.body).toMatch(/welcome to Loupe/);
      });
    });

    it("handle Readmore", function () {
      runs(function () {
        window.Mock.throwListCommentItemsLength = 10;
        dashboard = new Dashboard(initData);
      });
