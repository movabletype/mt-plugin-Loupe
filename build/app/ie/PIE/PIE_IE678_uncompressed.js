...
(function (win, doc) {
  ...
  PIE.GradientUtil = {
    ...
    getGradientMetrics: function (el, width, height, gradientInfo) {
      ...
      endPoint = PIE.GradientUtil.perpendicularIntersect(width / 2, height / 2, angle, (angle >= 180 ? 0 : width), (angle < 90 || angle > 270 ? 0 : height));
      endX = endPoint[0];
      endY = endPoint[1];
      startX = width - endX;
      startY = height - endY;
  ...
  PIE.BackgroundStyleInfo = PIE.StyleInfoBase.newStyleInfo({
    ...
    parseCss: function (css) {
      ...
      // Otherwise, use the standard background properties; let IE give us the values rather than parsing them
        ...
        this.withActualBg(PIE.ieDocMode < 9 ? function () {
          ...
          if (len && images[0] !== 'none') {
            ...
            for (i = 0; i < len; i++) {
              ...
              if (image && image !== 'none') {
                ...
                props.bgImages.push({
                  origString: image + ' ' + repeats[i] + ' ' + positions[i] + ' / ' + sizes[i] + ' ' + origins[i] + ' ' + clips[i],
                  imgType: 'image',
   ...
   */ (function () {
    ...
    PIE.BorderRadiusStyleInfo.ALL_ZERO = {
      x: zeros,
      y: zeros
    };
  ...
  PIE.RootRenderer = PIE.RendererBase.newRenderer({
    ...
    removeShape: function (shape) {
      var shapes = this._renderedShapes,
        i;
      if (shapes) {
  ...
  PIE.BackgroundRenderer = PIE.RendererBase.newRenderer({
      ...
      PIE.Util.withImageSize(src, function (imgSize) {
        ...
        if (elW && elH) {
          var styleInfos = me.styleInfos,
            ...
            bgPos = bg.bgPosition ? bg.bgPosition.coords(el, bgAreaSize.w - adjustedImgSize.w, bgAreaSize.h - adjustedImgSize.h) : {
              x: 0,
              y: 0
            },
            repeat = bg.imgRepeat,
            ...
            clipL = 0,
            clipR = elW + 1,
            clipB = elH + 1, //make sure the default clip region is not inside the box (by a subpixel)
    ...
    addLinearGradient: function (shape, info) {
      ...
      vmlAngle = (angle % 90) ?
        Math.atan2(metrics.startY - metrics.endY, (metrics.endX - metrics.startX) * w / h) / PI * 180 - 90 : -angle;
      while (vmlAngle < 0) {
        vmlAngle += 360;
      }
